...
export async function adaptTxtDocx(
  input: L0Input,
  options: L0Options = {},
): Promise<L0Result> {
...
async function extractText(input: L0Input): Promise<string> {
  if (typeof input === 'string') return input;
  if (isPathInput(input)) {
    if (/\.docx$/i.test(input.path)) return docxToText(await readPathBytes(input.path));
    return await readPathText(input.path); // .txt / .md / 其他文本文件
  }
  if (isBytesInput(input)) {
    if (isZipData(input)) return docxToText(input);
  ...
  if (isFilesInput(input)) {
    ...
    if (values.length === 1) {
      ...
      if (v instanceof Uint8Array) return isZipData(v) ? docxToText(v) : decodeUtf8(v);
    }
  }
  throw new Error('txt-docx 适配器：不支持的输入形态（期待 txt 文本、docx 字节或文件路径）');
}

/** docx 字节 → 全文文本（word/document.xml 段落行拼接） */
function docxToText(bytes: Uint8Array): string {
  const entries = readZipEntries(bytes);
  const doc = findZipEntry(entries, 'document.xml');
  if (!doc) throw new Error('txt-docx 适配器：docx 内未找到 word/document.xml');
...
export const txtDocxAdapter: L0Adapter = {
  name: 'txt-docx',
  label: '纯文本 / Word 聊天记录',
  description:
    'txt 直读 + docx 零依赖 zip+xml 提取；段头格式确定性解析（roleAliases）；无段头纯文本 LLM 分角色（BYO key，无 key 保留原文待标注不报错）',
