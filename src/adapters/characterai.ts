...
export async function adaptCharacterAI(input: L0Input): Promise<L0Result> {
  ...
  if (files.messages.length === 0) {
    throw new Error(
      'characterai 适配器：未识别到消息数据（期待官方导出 ZIP 内 message.json，或三文件内容映射）',
    );
  }

  const parsed: CAIParsedMessage[] = [];
  let skipped = 0;
  for (const item of files.messages) {
    ...
    const src =
      typeof entry.src === 'object' && entry.src !== null
        ? (entry.src as Record<string, unknown>)
        : null;
    ...
    if (isHuman === true) role = 'user';
    else if (isHuman === false) role = 'assistant';
    else if (speaker !== null && speaker === userName) role = 'user';
    else if (speaker !== null && speaker === characterName) role = 'assistant';
    else {
      role = 'unlabeled';
      pendingLabel = true;
    }
    ...
    const ts =
      parseIsoTs(firstNonEmptyString(entry.created_at, src?.created_at)) ??
  ...
  if (parsed.length > 1 && parsed.every((m) => m.ts !== null)) {
    parsed.sort((a, b) => (a.ts as number) - (b.ts as number));
    sortedByTime = true;
  }

  const messages: RawMessage[] = parsed.map((m) => ({
    role: m.role,
    text: m.text,
    ts: m.ts,
    meta: {
...
async function resolveCAIFiles(input: L0Input): Promise<CAIFiles> {
  // 1) 三文件映射（kind: 'files'）或已解析对象 {user, character, message(s)}
  if (isFilesInput(input)) return filesFromMap(input.files);
  if (typeof input === 'object' && input !== null && !isBytesInput(input)) {
    const obj = input as Record<string, unknown>;
    if (
      ['user', 'character', 'message', 'messages'].some((k) => k in obj) &&
      obj.kind === undefined
    ) {
  ...
  if (isBytesInput(input)) zipBytes = input;
    ...
    if (/\.zip$/i.test(input.path)) {
      ...
      if (isZipData(bytes)) zipBytes = bytes;
      else return { user: null, character: null, messages: parseMessageArray(decodeUtf8(bytes)) };
    }
  }
  if (zipBytes !== null) {
    ...
    if (!messageEntry) {
      throw new Error(
        'characterai 适配器：ZIP 内未找到 message.json（期待官方导出 ZIP：<用户名>/data/{user,character,message}.json）',
      );
    }
    return {
  ...
  throw new Error(
    'characterai 适配器：不支持的输入形态（期待 ZIP 字节/路径、三文件映射或 message.json 文本）',
  );
}

/** 文件映射 → 三文件解析（键匹配按基名/去 .json 词干；值可为文本/字节/已解析） */
function filesFromMap(files: Record<string, unknown>): CAIFiles {
  const pick = (names: string[]): unknown => {
    for (const key of Object.keys(files)) {
      const base = key.replace(/\\/g, '/').split('/').pop() ?? key;
      const stem = base.replace(/\.json$/i, '');
      if (names.includes(base) || names.includes(stem)) return files[key];
    }
    return undefined;
  };
  return {
...
function parseMaybeJson(v: unknown): unknown {
  ...
  if (v instanceof Uint8Array) return tryParseJson(decodeUtf8(v));
  return v;
}

function toJsonObject(v: unknown): Record<string, unknown> | null {
...
function toMessageArray(v: unknown): unknown[] {
  const parsed = parseMaybeJson(v);
  return Array.isArray(parsed) ? parsed : [];
}

function parseMessageArray(text: string): unknown[] {
  const parsed = tryParseJson(text);
  return Array.isArray(parsed) ? parsed : [];
}

function parseJsonObject(entry: { data: Uint8Array } | undefined): Record<string, unknown> | null {
...
export const characteraiAdapter: L0Adapter = {
  name: 'characterai',
  label: 'Character.AI 官方导出',
  description:
    '官方导出 ZIP：<用户名>/data/{user,character,message}.json 三文件关联映射；消息文本取 raw_content 字段；is_human/说话人名推断角色',
