...
export function isZipData(data: Uint8Array): boolean {
  ...
  if (data[2] === 0x03 && data[3] === 0x04) return true;
  if (data[2] === 0x05 && data[3] === 0x06) return true;
  return false;
}

/** 读取 ZIP 全部条目（按中央目录；stored/deflate；加密与 ZIP64 抛错） */
export function readZipEntries(data: Uint8Array): ZipEntry[] {
  if (!isZipData(data)) throw new Error('不是有效的 ZIP 数据（缺少 PK 文件头）');
  ...
  if (eocd < 0) throw new Error('ZIP 解析失败：未找到中央目录结束记录（EOCD）');
  ...
  for (let i = 0; i < entryCount; i++) {
    ...
    if ((flags & 0x1) !== 0) throw new Error(`ZIP 条目 "${name}" 使用了加密，无法读取`);
    ...
    if (method === 0) content = new Uint8Array(raw); // stored：原样
    else if (method === 8) content = new Uint8Array(inflateRawSync(raw)); // deflate
    else throw new Error(`ZIP 条目 "${name}" 使用压缩方式 ${method}，暂不支持`);
...
export function findZipEntry(entries: ZipEntry[], basename: string): ZipEntry | undefined {
 ...
 */
export function docxParagraphLines(xml: string): string[] {
  const lines: string[] = [];
