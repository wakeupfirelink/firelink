...
export async function adaptChatGPT(
  input: L0Input,
  options: L0Options = {},
): Promise<L0Result> {
  ...
  if (conversations.length === 0) {
    throw new Error(
      'chatgpt 适配器：未识别到会话数据（期待官方导出 conversations.json：含 mapping 的会话数组）',
    );
  }
  ...
  conversations.forEach((conv, conversationIndex) => {
    const linear =
      mode === 'current-path' ? currentPathMessages(conv) : treeLinearMessages(conv);
...
function extractConversations(parsed: unknown): ChatGPTConversation[] {
  ...
  if (parsed !== null && typeof parsed === 'object') {
    ...
    if (isConversation(obj)) return [obj];
  }
  return [];
}

function isConversation(v: unknown): v is ChatGPTConversation {
  return (
    typeof v === 'object' &&
    v !== null &&
    'mapping' in v &&
    typeof (v as ChatGPTConversation).mapping === 'object' &&
    (v as ChatGPTConversation).mapping !== null
  );
}

/** tree 模式：根节点 DFS 前序遍历（对象引用去重防环） */
function treeLinearMessages(conv: ChatGPTConversation): ChatGPTMessage[] {
  const mapping = conv.mapping ?? {};
  const out: ChatGPTMessage[] = [];
  const seen = new Set<ChatGPTMappingNode>();
  const visit = (node: ChatGPTMappingNode): void => {
  ...
  for (const node of Object.values(mapping)) {
    if (node.parent === null || node.parent === undefined) visit(node);
  }
  return out;
}

/** current-path 模式：current_node 沿 parent 回溯到根后反转 */
function currentPathMessages(conv: ChatGPTConversation): ChatGPTMessage[] {
  const mapping = conv.mapping ?? {};
  ...
  while (cursor !== undefined && cursor !== null && !seen.has(cursor)) {
    seen.add(cursor);
    if (cursor.message) path.push(cursor.message);
...
function extractMessage(msg: ChatGPTMessage): ExtractedMessage | null {
  const role = msg.author?.role;
  if (role === 'tool') return null; // 工具调用消息（内部 CoT 产物，不入记忆）
  ...
  const text = Array.isArray(parts)
    ? parts
        ...
        .join('\n')
    : '';
  if (text.length === 0) return null; // 空 system / 纯附件占位
  const mapped: MessageRole | null =
    role === 'user'
      ? 'user'
      : role === 'assistant'
        ? 'assistant'
...
export const chatgptAdapter: L0Adapter = {
  name: 'chatgpt',
  label: 'ChatGPT 官方导出',
  description:
    '官方 conversations.json（会话数组，mapping 消息树遍历→线性序列；tool 与空消息跳过；create_time 秒→ms）',
