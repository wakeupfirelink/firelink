...
import {
  ...
  type MultimodalTranscribeRequest,
  type RawMessage,
} from './common.js';
...
export const PROMPT_SCREENSHOT_TRANSCRIBE =
  '你是聊天记录截图的结构化转写助手。请把这张截图中的对话内容按从上到下的阅读顺序' +
...
/* ================================================================
 * 二、多模态 provider（BYO key，OpenAI 兼容视觉端点实现）
 * ===============================================================*/

export interface OpenAIVisionOptions {
  /** 端点基址，如 https://api.example.com（自动拼 /chat/completions） */
  baseUrl: string;
  apiKey: string;
  model: string;
  temperature?: number;
...
export class OpenAIVisionProvider implements MultimodalProvider {
  readonly name: string;
  readonly model: string;
  private readonly opts: OpenAIVisionOptions;

  constructor(opts: OpenAIVisionOptions) {
    this.opts = opts;
    this.name = normalizeEndpointName(opts.baseUrl);
    this.model = opts.model;
  }

  async transcribe(req: MultimodalTranscribeRequest): Promise<string> {
    ...
    try {
      const dataUri = `data:${req.mimeType};base64,${Buffer.from(req.image).toString('base64')}`;
      ...
      if (typeof content !== 'string') throw new Error('多模态端点响应缺少 choices[0].message.content');
      return content;
    } finally {
      clearTimeout(timer);
    }
...
function mimeFromPath(path: string): string | null {
  ...
  if (ext === 'png' || ext === 'jpg' || ext === 'jpeg') return ext === 'png' ? 'image/png' : 'image/jpeg';
...
export async function adaptScreenshot(
  input: L0Input,
  options: L0Options = {},
): Promise<L0Result> {
  ...
  if (!provider) {
    throw new Error(
      '截图转写需要多模态 LLM provider（BYO key）：请在 options.multimodal 注入实现 MultimodalProvider 的实例',
    );
  }
...
async function resolveImage(input: L0Input): Promise<{ image: Uint8Array; mimeType: string }> {
...
function normalizeSimpleRole(role: unknown): MessageRole {
...
export const screenshotAdapter: L0Adapter = {
  name: 'screenshot',
  label: '截图转写（多模态）',
  description:
    '截图 → 多模态 LLM 转写（BYO key，provider 可插拔；内置 OpenAI 兼容视觉端点实现与转写提示词模板）',
