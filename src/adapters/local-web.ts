...
export function extractHtmlTextBlocks(html: string): HtmlExtractResult {
...
export async function adaptLocalWeb(
  input: L0Input,
  options: L0Options = {},
): Promise<L0Result> {
  let html: string;
  if (typeof input === 'string') html = input;
  else if (isPathInput(input)) html = await readPathText(input.path);
  else if (isBytesInput(input)) html = decodeUtf8(input);
  else throw new Error('local-web 适配器：不支持的输入形态（期待 HTML 文本或本地页面路径）');
...
export const localWebAdapter: L0Adapter = {
  name: 'local-web',
  label: '本地保存的 HTML 页面',
  description:
    '关停平台兜底路径：另存页面正文提取（script/style 剔除、块级切行、实体解码）；段头确定性解析或 LLM 分角色（BYO key）',
