...
/* ================================================================
 * 一、配置模型（噪音表 / 去重 / 脱敏规则，全部可 JSON 配置）
 * ===============================================================*/
...
export interface NoiseRule {
   ...
   * exact：段正文全等命中；
   * contains：段正文包含子串命中；
   * regex：段正文正则命中（作用于整段文本，多行模式）
   */
  type: 'exact' | 'contains' | 'regex';
  /** exact/contains 为字面文本；regex 为正则源串 */
  pattern: string;
  /** regex 附加 flags（默认含 g 与 m 无关——只做 test，不做替换） */
  flags?: string;
  /** 规则标签（统计用） */
  label?: string;
}
...
export interface DedupConfig {
  /** 总开关（默认开） */
  enabled: boolean;
  /** 滑窗长度：连续 windowSize 条段指纹序列整体重复才判定为重复段（默认 5） */
  windowSize: number;
  /**
   * 参与去重的段最小长度（默认 4 字符）：短于此的段（嗯/好/在吗等自然
   * 短句）不参与窗口判定——防周期性短句交替被误判为「重复段」。
   * 重复段去重的语义目标是「大段重复导出」，不是自然短句复读。
   */
...
export interface RedactionRule {
  /** 规则名（命中统计键，如 phone / id_card / student_id） */
  name: string;
  /** 正则源串（JS RegExp 文法；建议带前后断言防误匹配） */
  pattern: string;
  flags?: string;
  /** 替换标记（如「[已脱敏:手机号]」） */
  replacement: string;
...
/** 消息边界切分配置（纯文本源的消息头模式，如 docx 导出的「名字: 06-13 22:43:21 」） */
export interface MessageBoundaryConfig {
  /** 消息头正则源串（捕获组：1=说话人名，2=月，3=日，4=时，5=分，6=秒[可选]） */
  pattern: string;
  flags?: string;
}
...
export interface CleanConfig {
  ...
  /** 重复段去重 */
  dedup: DedupConfig;
  /** 正则脱敏规则表（按声明顺序执行——长模式在前防部分消费） */
  redactionRules: RedactionRule[];
  /** 是否清理 Unicode 控制字符与零宽字符（默认开） */
  stripControlChars: boolean;
  /** 消息边界切分（可选；提供后纯文本按消息头切段，供 L2 复用 header） */
  messageBoundary?: MessageBoundaryConfig;
}
...
export const DEFAULT_REDACTION_RULES: RedactionRule[] = [
  {
    name: 'id_card_18',
    pattern:
      '(?<!\\d)\\d{6}(?:19|20)\\d{2}(?:0[1-9]|1[0-2])(?:0[1-9]|[12]\\d|3[01])\\d{3}[\\dXx](?!\\d)',
    flags: 'g',
...
export const DEFAULT_MESSAGE_BOUNDARY: MessageBoundaryConfig = {
  pattern:
    '([^\\s:：][^\\s:：]{0,19})\\s*[:：]\\s*(\\d{1,2})-(\\d{1,2})\\s+(\\d{1,2}):(\\d{2})(?::(\\d{2}))?\\s+',
  flags: 'g',
};
...
export function mergeCleanConfig(partial?: Partial<CleanConfig>): CleanConfig {
  if (!partial) return structuredCloneConfig(DEFAULT_CLEAN_CONFIG);
  return {
 ...
 */
export function loadCleanConfig(path: string): CleanConfig {
  const raw = JSON.parse(readFileSync(path, 'utf-8')) as Partial<CleanConfig>;
  return mergeCleanConfig(raw);
}

function structuredCloneConfig(config: CleanConfig): CleanConfig {
  return {
...
export interface Segment {
  /** 消息头原文（如「星见雅: 06-13 22:43:21」）；非消息边界的散文本段为 null */
  header: string | null;
  /** 段正文 */
  text: string;
  /**
   * 原始结构化消息引用（输入为对象数组时保留，供 L2 直映射还原
   * role/ts/meta——清洗只改写 text 与段去留，不吞结构化信息）。
   * 鸭子类型最小面 { text: string }，本层不感知具体形态（零依赖）。
   */
  ref?: unknown;
}
  ...
  if (Array.isArray(input)) {
    ...
    for (const item of input) {
      if (typeof item !== 'string') {
        ...
        if (t.length > 0) segments.push({ header: null, text: t, ref: item });
        continue;
      }
...
/** 按消息边界正则切段 */
function splitByBoundary(text: string, boundary: MessageBoundaryConfig): Segment[] {
  ...
  for (let i = 0; i < matches.length; i++) {
    ...
    if (body.length === 0 && matches[i][1].trim().length === 0) continue;
...
function isCleanConfig(c?: CleanConfig | Partial<CleanConfig>): c is CleanConfig {
  return !!c && Array.isArray(c.noiseRules) && Array.isArray(c.redactionRules) && !!c.dedup;
}

/** 噪音判定 */
function isNoise(segment: Segment, config: CleanConfig): boolean {
  const text = segment.text.trim();
  if (text.length === 0) return true;
  for (const rule of config.noiseRules) {
    if (rule.type === 'exact' && text === rule.pattern) return true;
    if (rule.type === 'contains' && text.includes(rule.pattern)) return true;
    if (rule.type === 'regex') {
...
export function dedupeSegments(
  ...
  if (windowSize < 2 || n < windowSize * 2) return segments;
  ...
  const isShort = (i: number) => segments[i].text.trim().length < minSegmentLength;
  ...
  for (let i = 0; i + windowSize <= n; i++) {
    ...
    for (let j = i; j < i + windowSize; j++) {
      if (!isShort(j)) { allShort = false; break; }
    }
    if (allShort) continue;
    const key = hashes.slice(i, i + windowSize).join('|');
    if (seen.has(key)) {
      for (let j = i; j < i + windowSize; j++) removed[j] = true;
    } else {
      seen.add(key);
    }
  }
...
function applyRedaction(
  ...
  for (const rule of rules) {
    ...
    if (count > 0) hits[rule.name] = (hits[rule.name] ?? 0) + count;
  }
  return out;
}

/* ================================================================
 * 六、LLM 复核层（可选，BYO key）
 * ===============================================================*/

/** LLM 复核命中（LLM 返回的脱敏后段全文） */
export interface LlmRedactHit {
  /** 段在输入数组中的下标 */
  index: number;
  /** LLM 给出的脱敏后段全文 */
  text: string;
 ...
 */
export async function llmRedactReview(
  segments: Segment[],
  provider: { chat(req: { system?: string; user: string; json?: boolean }): Promise<string> } | undefined,
  options?: { batchSize?: number },
): Promise<LlmRedactResult> {
  if (!provider || segments.length === 0) {
    return { segments, hits: [], stats: { batches: 0, replaced: 0 } };
  }
  const batchSize = options?.batchSize ?? 40;
  const out = segments.map((s) => ({ ...s }));
  const hits: LlmRedactHit[] = [];
  let batches = 0;
  ...
  for (let base = 0; base < segments.length; base += batchSize) {
    ...
    batches++;
    const parsed = extractJson(raw) as { hits?: LlmRedactHit[] } | null;
    if (!parsed || !Array.isArray(parsed.hits)) continue;
