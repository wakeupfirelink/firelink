...
/** LLM provider 最小抽象（结构满足即可插拔：DeepSeek/任意 OpenAI 兼容端点/mock） */
...
/** OpenAI 兼容端点 provider（/chat/completions 协议） */
export interface OpenAICompatibleOptions {
  /** 端点基址，如 https://api.deepseek.com（自动拼 /chat/completions） */
  baseUrl: string;
  apiKey: string;
  model: string;
  temperature?: number;
  timeoutMs?: number;
  /** fetch 注入（测试 mock 用；默认 globalThis.fetch，Node 18+ 内置） */
...
function normalizeEndpointName(baseUrl: string): string {
  try {
...
export function createProviderFromEnv(
  ...
  if (env.MEMORY_LLM_API_KEY) {
    return new OpenAICompatibleProvider({
      baseUrl: env.MEMORY_LLM_BASE_URL ?? 'https://api.deepseek.com',
...
export interface LongTermItem {
  text: string;
  category?: 'identity' | 'relationship' | 'core_fact';
}

/** 笔记条目（→ L4 笔记本库，text ≤50 字由 L4 校验/截断） */
export interface NotebookItem {
  text: string;
  score?: number;
  category?: string;
  /** 元认知批注（≤30 字，议题 28；L4 截断） */
  reason?: string;
}

/** 画像信号（→ L4 用户画像库） */
export interface ProfileSignal {
  dimension: 'preferences' | 'personality' | 'habits';
  key: string;
  value: string;
}
...
export interface DistillResult {
  /** true = 无 provider 自动跳过（36 施工案降级拍板） */
  skipped: boolean;
  skipReason?: string;
...
export interface DistillOptions {
  ...
  now?: () => number;
  /** 单周消息正文长度上限（超出截断，防 token 超限；默认 12000 字符） */
  maxCharsPerWeek?: number;
}
...
/** 无 provider 的降级结果工厂 */
export function skippedDistill(now: () => number = Date.now): DistillResult {
  return {
    skipped: true,
...
export async function distill(
  ...
  if (!provider) return skippedDistill(options.now ?? Date.now);
...
export async function distillWeek(
  week: WeekBatch,
  provider: LLMProvider,
  options: DistillOptions = {},
): Promise<{ summary: WeeklySummary; anchors: Anchor[]; llmCalls: number }> {
  ...
  const summaryParsed = extractJson(summaryRaw) as { summary?: string } | null;
  ...
  const anchorParsed = extractJson(anchorRaw) as { anchors?: unknown[] } | null;
  const anchors: Anchor[] = Array.isArray(anchorParsed?.anchors)
...
/** 分层标签（三路泛化输出） */
export async function distillLayers(
  weeklySummaries: WeeklySummary[],
  anchors: Anchor[],
  provider: LLMProvider,
  now: () => number = Date.now,
): Promise<{ longTerm: LongTermItem[]; notebook: NotebookItem[]; profile: ProfileSignal[]; generatedAt: string }> {
...
export function groupByWeek(messages: UnifiedMessage[]): WeekBatch[] {
  const buckets = new Map<string, WeekBatch>();
  for (const m of messages) {
    ...
    if (m.ts !== null) {
      ...
      if (iso < bucket.start) bucket.start = iso;
      if (iso > bucket.end) bucket.end = iso;
    }
  }
  // 按周键排序（unknown 在最后）
  return [...buckets.values()].sort((a, b) =>
    a.weekKey === 'unknown' ? 1 : b.weekKey === 'unknown' ? -1 : a.weekKey.localeCompare(b.weekKey),
  );
}
...
/** 消息转录（时间线文本；超长截断） */
export function renderTranscript(messages: UnifiedMessage[], maxChars: number): string {
  const lines: string[] = [];
  let used = 0;
  let truncated = 0;
  for (const m of messages) {
  ...
  if (truncated > 0) lines.push(`（另有 ${truncated} 条消息因长度上限未纳入转录）`);
  return lines.join('\n');
}
...
function isAnchorShape(v: unknown): v is { date: string; ts: number | null; event: string; importance: number } {
...
function isLongTermShape(v: unknown): v is { text: string; category?: 'identity' | 'relationship' | 'core_fact' } {
  const a = v as Record<string, unknown>;
...
function isNotebookShape(v: unknown): v is {
  text: string;
  score?: number;
  category?: string;
  reason?: string;
...
function isProfileShape(v: unknown): v is {
  dimension: 'preferences' | 'personality' | 'habits';
  key: string;
  value: string;
} {
  const a = v as Record<string, unknown>;
  return (
    (a?.dimension === 'preferences' || a?.dimension === 'personality' || a?.dimension === 'habits') &&
    typeof a?.key === 'string' &&
