 ...
 */
import { buildL0Result, resolveInputText, tryParseJson, } from './common.js';
/* ================================================================
 * 一、主转换
 * ===============================================================*/
export async function adaptStChatlog(input) {
    ...
    for (const line of lines) {
        ...
        if (!('mes' in rec)) {
            if ('user_name' in rec || 'character_name' in rec) {
                if (typeof rec.user_name === 'string')
                    chatMeta.user_name = rec.user_name;
                if (typeof rec.character_name === 'string')
                    chatMeta.character_name = rec.character_name;
                continue;
            }
            skipped++;
            continue;
...
function parseSendDate(value) {
    ...
    if (s.length === 0)
        return null;
    if (/^\d+(\.\d+)?$/.test(s))
        return epochNormalize(Number(s));
 ...
 * 三、适配器对象
 * ===============================================================*/
export const stChatlogAdapter = {
    name: 'st-chatlog',
    label: 'SillyTavern 聊天记录',
    description: '社区标准 jsonl 格式（每行 {name, is_user, is_system, send_date, mes}；send_date 支持 epoch 秒/毫秒、ISO、人性化日期）',
